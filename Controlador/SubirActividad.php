        ...
        if(filter_var($codigo, FILTER_VALIDATE_INT) && filter_var($idCursoTarea, FILTER_VALIDATE_INT))
        {
            ...
            if($queryConsultaCodigo == null){
                ...
                if(filter_var($tipoActividad, FILTER_VALIDATE_INT) <> 3 and $queryConsultaTipoActividad != null){
                    echo "<script>alert('NO se subió la actividad: Ya hay un registro subido para esa actividad puntual.');window.location='../Vista/Paginas-Admin/Tareas-admin.php?idCurso=".$idCursoTarea."'</script>";
                }else{
                    ...
                    if($resultado){
                        ...
                        $route_actividad="../Vista/img/Tareas/".$actividad;
                        move_uploaded_file($actividad_tmp,$route_actividad);
                        echo "<script>alert('se ha enviado informe');window.location='../Vista/Paginas-Admin/Tareas-admin.php?idCurso=".$idCursoTarea."'</script>";
                    } else {
                ...
                echo "<script>alert('NO se subió la actividad: El código de la actividad ya fue registrado con anterioridad.');window.location='../Vista/Paginas-Admin/Tareas-admin.php?idCurso=".$idCursoTarea."'</script>";
            }
        }else{
            echo "<script>alert('NO se subió la actividad: El código de la actividad debe ser un valor entero diferente de 0.');window.location='../Vista/Paginas-Admin/Tareas-admin.php?idCurso=".$idCursoTarea."'</script>";
        }
    }
    else{
